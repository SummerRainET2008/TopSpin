...
from palframe.tf_1x.capsule_net.utils import _conv2d_wrapper, _get_weights_wrapper
...
def routing(u_hat_vecs, iterations):
  ...
  origin_shape = u_hat_vecs.get_shape().as_list()
  u_shape = u_hat_vecs.get_shape().as_list()
  u_hat_vecs = tf.reshape(
    u_hat_vecs, (u_shape[0] * u_shape[1], u_shape[2], u_shape[3])
  )

  for i in range(iterations):
    c = softmax(b, 1)
    c_shape = c.get_shape().as_list()
    c = tf.reshape(c, (c_shape[0] * c_shape[1], c_shape[2]))
    outputs = squash_v1(K.batch_dot(c, u_hat_vecs, [1, 1]))
    if i < iterations - 1:
      b_shape = b.get_shape().as_list()
      b = tf.reshape(b, (b_shape[0] * b_shape[1], b_shape[2]))
      b = b + K.batch_dot(outputs, u_hat_vecs, [1, 2])
      b = tf.reshape(b, (origin_shape[0], origin_shape[1], b_shape[2]))

  out_shape = outputs.get_shape().as_list()
  outputs = tf.reshape(
    outputs, (origin_shape[0], origin_shape[1], out_shape[-1])
  )
  poses = outputs
...
def vec_transformation_by_conv(poses, input_capsule_dim, input_capsule_num,
  ...
  u_hat_vecs = keras.backend.reshape(
    u_hat_vecs, (
      -1, input_capsule_num, output_capsule_num, output_capsule_dim
    )
  )
  u_hat_vecs = keras.backend.permute_dimensions(u_hat_vecs, (0, 2, 1, 3))
  return u_hat_vecs

def vec_transformation_by_mat(poses, input_capsule_dim,
                              input_capsule_num, output_capsule_dim,
                              output_capsule_num, shared=True):
  ...
  if shared:
    kernel = _get_weights_wrapper(
      name='weights',
      shape=[1, 1, output_capsule_num, output_capsule_dim, input_capsule_dim],
      weights_decay_factor=0.0
    )
    kernel = tf.tile(
    ...
    kernel = _get_weights_wrapper(
      name='weights',
      shape=[
        1, input_capsule_num, output_capsule_num,
        output_capsule_dim, input_capsule_dim
      ],
      weights_decay_factor=0.0
    )
    kernel = tf.tile(
