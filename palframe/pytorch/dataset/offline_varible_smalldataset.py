...
class MyBatchSampler:
  def __init__(self, dataset, variable_batch_size: dict, length_key_func,
               shuffle: bool):
    items = self._create_group_data(
      dataset._data, variable_batch_size, length_key_func, shuffle
    )
  ...
  def __iter__(self):
    group_iters = [nlp.next_batch(data_ids, self._group_batch_size[gid][1])
...
def get_batch_data(feat_path,
  ...
  for epoch_id in range(epoch_num):
    my_bacth_sampler = MyBatchSampler(
      dataset, variable_batch_size, length_key_func, shuffle
    )
