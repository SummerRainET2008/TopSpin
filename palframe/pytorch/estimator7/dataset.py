...
from palframe.pytorch.estimator7.file_io import load_file
...
class OnlineDataset(_OnlineDataset):
  ...
  def _gen_from_files(self, files: list):
    for fname in files:
      ...
      buff = load_file(fname, file_extension=self.file_extension)
...
class OfflineDataset(_OfflineDataset):
  ...
  def _load_data(self, all_feat_files, world_size, rank, sample_filter_func):
    ...
    data = []
    for f in feat_files:
      data.extend(load_file(f, self.file_extension))
