  ...
  def __init__(self,
    ...
    if optimizer is not None:
      ...
      self._optimizer = getattr(torch.optim, param.optimizer_name)(
        model_wrapper._model.parameters(), lr=param.lr, weight_decay=param.l2
      )

    batch_num_per_epoch = math.ceil(
    ...
    self._scheduler = get_cosine_schedule_with_warmup(
      self._optimizer, warmup_steps, total_steps
    )
    ...
    if nlp.is_none_or_empty(param.vali_file):
      ...
      with torch.no_grad():
        ...
        self._writer.add_scalar(
          f"eval '{param.vali_file}'", eval_error, self._global_step_id
        )
  ...
  def _evaluate(self):
    ...
    for data_file in self._model_wrapper._param.test_files:
      with torch.no_grad():
        ...
        self._writer.add_scalar(
          f"eval '{data_file}'", eval_error, self._global_step_id
        )
  ...
  def train(self):
    ...
    while True:
      ...
      for _, [epoch_id, batch] in zip(
        range(param.iter_num_update_optimizer), batch_iter
      ):
        if self.use_amp:
          with amp.autocast():
      ...
      Logger.info(
        f"*Epoch: {self._run_sample_num // param.train_sample_num} "
