...
def fetch_google_bert_model(model_name: str, fetch_dir: str):
  if model_name not in bert_models_google:
    raise ValueError(
        "BERT model with name:[{}] not found, try one of:{}".format(
...
def params_from_pretrained_ckpt(bert_ckpt_dir):
  json_config_files = tf.io.gfile.glob(
      os.path.join(bert_ckpt_dir, "*_config*.json"))
  if len(json_config_files) != 1:
    raise ValueError(
        "Can't glob for BERT config json at: {}/*_config*.json".format(
            bert_ckpt_dir))
...
def bert_prefix(bert: BertModelLayer):
  ...
  assert match, "Unexpected bert layer: {} weight:{}".format(
      bert, bert.weights[0].name)
...
def load_stock_weights(bert: BertModelLayer, ckpt_path):
  ...
  assert _checkpoint_exists(ckpt_path), "Checkpoint does not exist: {}".format(
      ckpt_path)
  ...
  print("Done loading {} BERT weights from: {} into {} (prefix:{}). "
        ...
        "Count of weights with mismatched shape: [{}]".format(
            len(weight_value_tuples), ckpt_path, bert, prefix, skip_count,
