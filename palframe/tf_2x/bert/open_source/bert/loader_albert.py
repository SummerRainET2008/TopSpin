...
from palframe.tf_2x.bert.open_source.bert import BertModelLayer, loader

albert_models_tfhub = {
    "albert_base":
    "https://tfhub.dev/google/albert_base/{version}?tf-hub-format=compressed",
    "albert_large":
    "https://tfhub.dev/google/albert_large/{version}?tf-hub-format=compressed",
    "albert_xlarge":
    "https://tfhub.dev/google/albert_xlarge/{version}?tf-hub-format=compressed",
    "albert_xxlarge":
    "https://tfhub.dev/google/albert_xxlarge/{version}?tf-hub-format=compressed",
}

albert_models_brightmart = {
    "albert_tiny":
    "https://storage.googleapis.com/albert_zh/albert_tiny.zip",
    "albert_tiny_489k":
    "https://storage.googleapis.com/albert_zh/albert_tiny_489k.zip",
    "albert_base":
...
def fetch_brightmart_albert_model(model_name: str, fetch_dir: str):
  if model_name not in albert_models_brightmart:
    raise ValueError(
        "ALBERT model with name:[{}] not found at brightmart/albert_zh, try one of:{}"
        .format(model_name, albert_models_brightmart))
  else:
...
def fetch_tfhub_albert_model(albert_model: str, fetch_dir: str, version="2"):
  ...
  if albert_model.startswith("http"):
    ...
    raise ValueError(
        "ALBERT model with name:[{}] not found in tfhub/google, try one of:{}".
        format(albert_model, albert_models_tfhub))
  else:
  ...
  fetched_file = pf.utils.fetch_url(fetch_url,
                                    fetch_dir=fetch_dir,
...
def map_to_tfhub_albert_variable_name(name, prefix="bert"):
  ...
  name = re.compile("encoder/layer_shared/intermediate/(?=kernel|bias)").sub(
      "encoder/transformer/group_0/inner_group_0/ffn_1/intermediate/dense/",
      name)
  ...
  name = name.replace(
      "encoder/layer_shared/output/dense",
      "encoder/transformer/group_0/inner_group_0/ffn_1/intermediate/output/dense"
  )
...
def _is_tfhub_model(tfhub_model_path):
  try:
    ...
    variables_files = tf.io.gfile.glob(
        os.path.join(tfhub_model_path, "variables/variables.*"))
    pb_files = tf.io.gfile.glob(os.path.join(tfhub_model_path, "*.pb"))
  except tf.errors.NotFoundError:
    assets_files, variables_files, pb_files = [], [], []

  return len(pb_files) >= 2 and len(assets_files) >= 1 and len(
      variables_files) >= 2
...
def load_albert_weights(bert: BertModelLayer, tfhub_model_path, tags=[]):
  ...
  assert isinstance(
      bert,
      BertModelLayer), "Expecting a BertModelLayer instance as first argument"
  ...
  print("Done loading {} BERT weights from: {} into {} (prefix:{}). "
        ...
        "Count of weights with mismatched shape: [{}]".format(
            len(weight_value_tuples), tfhub_model_path, bert, prefix,
            skip_count, len(skipped_weight_value_tuples)))
  print(
